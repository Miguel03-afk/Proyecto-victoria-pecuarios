...
export default function Navbar() {
  ...
  const handleBuscar = (e) => {
    if (e.key === "Enter" && busqueda.trim()) navigate(`/?buscar=${busqueda.trim()}`);
  };
        ...
        <div className="flex items-center gap-1.5 flex-shrink-0">
          {usuario ? (
            ...
            <div className="flex items-center gap-1.5">
              <Link to="/login"
                className="text-xs text-gray-600 hover:text-green-700 font-medium px-3 py-1.5 rounded-lg hover:bg-green-50 transition-colors">
                Iniciar sesión
              </Link>
              <Link to="/registro"
                className="text-xs bg-green-600 hover:bg-green-700 text-white font-semibold px-3.5 py-1.5 rounded-lg transition-colors shadow-sm">
                Registrarse
              </Link>
            </div>
          )}
