    ...
    <>
      <style>{`
        ...
        @media (max-width: 640px) {
          .vp-agenda-vet-grid { grid-template-columns: 1fr; gap: 10px; }
          .vp-agenda-vet-btn  { padding: 14px !important; border-radius: 12px !important; }
        }
      `}</style>
      <div className="vp-agenda-vet-grid">
        {vets.map(v => (
          <button
            key={v.id}
            onClick={() => onElegir(v)}
            className="vp-agenda-vet-btn"
            style={{
...
function PasoFechaHora({ vet, fecha, setFecha, hora, setHora }) {
  ...
  // El click solo actualiza el estado — el fetch lo maneja el useEffect.
  // Así evitamos duplicar la lógica y aseguramos que cambios de fecha por
  // cualquier vía (botón, prefill, volver atrás) re-cargen los slots.
  const elegirFecha = (f) => {
    if (f === fecha) return; // ya está seleccionada, evita llamada redundante
    setFecha(f);
    setHora("");
  };

  // Fetch reactivo: cada vez que fecha o vet cambien, re-pedimos los slots.
  // Usa una bandera `cancelled` para evitar race conditions: si el usuario
  // hace click rápido en dos fechas, la respuesta de la primera NO sobreescribe
  // los slots de la segunda.
  useEffect(() => {
    if (!fecha || !vet?.id) {
      setSlots([]); setSinSlots(false); setCargando(false);
      return;
    }
    let cancelled = false;
    setCargando(true); setSlots([]); setSinSlots(false);

    api.get("/citas/disponibilidad", { params: { veterinario_id: vet.id, fecha } })
      .then(r => {
        if (cancelled) return; // descarta respuesta obsoleta
        const arr = r.data.slots || [];
        setSlots(arr);
        setSinSlots(arr.length === 0);
      })
      .catch(() => {
        if (cancelled) return;
        setSinSlots(true);
            ...
            return (
              <div style={{
                ...
                <label style={{
                  ...
                  letterSpacing: 0.8, color: C.textTer,
                }}>
                  Horarios para <strong style={{ color: C.brand }}>{DIAS_FULL[d.getDay()]} {d.getDate()} de {MESES_LARGO[d.getMonth()]}</strong>
                </label>
                {!cargando && !sinSlots && slots.length > 0 && (
