...
const Msg = ({texto, tipo="ok"}) => {
  ...
  // key={texto} fuerza re-mount al cambiar el mensaje → re-dispara la animación.
  return (
    <div key={texto}
      className="px-4 py-3 rounded-xl text-sm font-medium flex items-center gap-2"
      style={{
        ...colors,
        animation: "vp-msg-in 220ms cubic-bezier(0.23, 1, 0.32, 1)",
      }}>
      {tipo==="ok" ? "✓" : "✕"} {texto}
    </div>
  );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      style={{
        ...
        // Backdrop fade (separado del modal para no escalar el blur)
        animation: "vp-backdrop-in 180ms cubic-bezier(0.23, 1, 0.32, 1)",
      }}
      onClick={onClose}>
      <div className={`w-full ${ancho} max-h-[90vh] overflow-y-auto rounded-2xl`}
        style={{
          ...
          // Modal entrance: scale(0.95)+opacity (nunca scale(0)).
          // transform-origin: center es correcto para MODALES (no para popovers).
          transformOrigin: "center",
          animation: "vp-modal-in 200ms cubic-bezier(0.23, 1, 0.32, 1)",
