...
function Agenda() {
  ...
  // OJO: toISOString() devuelve UTC — en Colombia (UTC-5) después de las 7pm
  // la fecha "salta" al día siguiente y oculta las citas del día real.
  // Usamos getFullYear/getMonth/getDate que respetan la zona horaria local.
  ...
  // `citas` ya está filtrado a hoy arriba — no necesitamos otro filtro.
  // Solo agrupamos por estado para los contadores del header.
  const confirmadasHoy = citas.filter(c => c.estado === "confirmada").length;
  const pendientesHoy  = citas.filter(c => c.estado === "pendiente").length;

  // Citas próximas (futuras) con detalle — útil cuando hoy está vacío.
  // Sólo confirmadas/pendientes (excluyendo canceladas/rechazadas/completadas).
  const ESTADOS_VISIBLES = ["confirmada", "pendiente"];
  const proximasCitas = citasTodas
    .filter(c => c.fecha > hoyStr && ESTADOS_VISIBLES.includes(c.estado))
    .slice(0, 8); // mostrar máximo 8 para no abrumar
  const citasPasadas = citasTodas.filter(c => c.fecha < hoyStr).length;
            ...
            <div style={{ padding: "24px 18px" }}>
              <div style={{ textAlign: "center", marginBottom: 18 }}>
                <FontAwesomeIcon icon={faCalendarXmark} style={{
                  fontSize: 28, color: C.muted, marginBottom: 8, display: "block",
                }}/>
                <div style={{ fontWeight: 600, color: C.ink2, fontSize: 14, marginBottom: 4 }}>
                  No tienes citas hoy
                </div>
                <div style={{ fontSize: 11, color: C.ink3, fontFamily: FONT.mono }}>
                  {hoyStr}
                </div>
              </div>

              {/* Próximas citas confirmadas — el vet quiere verlas aquí */}
                ...
                <div>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    {proximasCitas.map(c => {
                      ...
                      const diaSem = fechaObj.toLocaleDateString("es-CO", { weekday: "short" });
                      const diaNum = fechaObj.getDate();
                      const mesCorto = fechaObj.toLocaleDateString("es-CO", { month: "short" });
                      ...
                      return (
                        <div key={c.id} style={{
                          ...
                          <div style={{
                            flexShrink: 0, textAlign: "center", minWidth: 38,
                            padding: "4px 6px", borderRadius: RADIUS.sm,
                            background: esConfirmada ? C.brandSoft : C.warnBg,
                            border: `1px solid ${esConfirmada ? C.brandBorder : C.warnBorder || "#fde68a"}`,
                          }}>
                            <div style={{ fontSize: 9, color: C.ink3, textTransform: "uppercase", fontWeight: 700 }}>
                              {diaSem}
                            </div>
                            <div style={{ fontSize: 14, fontWeight: 800, color: esConfirmada ? C.brand : C.warn, lineHeight: 1 }}>
                              {diaNum}
                            </div>
                            <div style={{ fontSize: 8, color: C.ink3, textTransform: "uppercase" }}>
                              {mesCorto}
                            </div>
                          </div>
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ fontWeight: 700, color: C.ink, marginBottom: 2 }}>
                              {especieEmoji(c.especie_mascota)} {c.nombre_mascota}
                            </div>
                            <div style={{ fontSize: 10, color: C.ink3 }}>
                              {fmtHora(c.hora)} · {esConfirmada ? "Confirmada" : "Pendiente"}
                            </div>
                          </div>
                        </div>
                      );
              ...
              {/* Resumen pasadas (más sutil, solo cuenta) */}
              {citasPasadas > 0 && (
                <div style={{
                  marginTop: 14, fontSize: 11, color: C.ink3,
                  textAlign: "center", padding: "8px 0",
                  borderTop: `1px solid ${C.line}`,
                }}>
                  {citasPasadas} cita{citasPasadas !== 1 ? "s" : ""} de fechas pasadas
                </div>
              )}
              ...
              {citasTodas.length === 0 && (
                <div style={{
                  ...
                  background: C.surfaceAlt, borderRadius: RADIUS.md,
                }}>
                  Aún no hay citas asignadas a tu perfil.
                </div>
              )}
            </div>
          ) : (
